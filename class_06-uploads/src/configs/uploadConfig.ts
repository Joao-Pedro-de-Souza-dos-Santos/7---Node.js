...
const fileFilter = 
    (_req: Request, file: Express.Multer.File, callback: FileFilterCallback) =>
    {
        ...
        const formatFiles = ["image.jpg", "image/jpeg", "image/pjpeg", "image/png", "image/gif"];

        if (formatFiles.includes(file.mimetype)) return callback(null, true);

        const error = new Error("Invalid Format!") as Error & { status: number };
        error.status = 400;
...
export const MULTER = {
            ...
            fileFilter,
            limits: {
                fileSize: 1024 * 1024 * 2 // 2 MegaByts
            }
        }